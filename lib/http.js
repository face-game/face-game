  ...
  constructor (port, github) {
    ...
    vm.apiRouter.post('/0/submit', async function (req, res) {
      ...
      let valid = Joi.validate(req.body, Joi.object().keys({
        ...
        devicePixelRatio: Joi.number().greater(0).positive().required(),
        images: Joi.array().items(Joi.object().keys({
          level: Joi.number().integer().min(1).max(2).required(),
          img: Joi.string().dataUri().required()
        })).length(2).unique().required()
      }))
      if (valid.error) {
        log.debug('invalid', valid.error, req.body)
        res.status(400).send('Invalid request')
      } else {
        let data = valid.value
        data.id = uuidv4()
        ...
        try {
          ...
          for (let img of data.images) {
            let fileReq = {
              ...
              message: `created: ${data.created.toString()}\nuserAgent: ${data.userAgent}\ndevicePixelRatio: ${data.devicePixelRatio}`,
          ...
          let pullReq = {
            owner: 'face-game',
            repo: 'face-game-content',
            title: `[new content] ${data.created.toString()} ${data.id}`,
            head: data.branch,
            base: 'master',
