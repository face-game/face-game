  ...
  constructor (port, github) {
    ...
    vm.apiRouter.post('/0/submit', async function (req, res) {
      ...
      try {
        ...
        log.debug('getRefRes', getRefReq, getRefRes)
        ...
        log.debug('newRefRes', newRefReq, newRefRes)
        ...
        log.debug('contentRes', contentReq, contentRes)
        ...
        if (contentRes.data.hasOwnProperty('content')) {
          ...
          log.debug('fileRes', gitFileReq, gitFileRes)
        } else {
        ...
        log.debug('pullRes', pullReq, pullRes)
      } catch (err) {
        log.error(err)
      }
    ...
    vm.app.use((req, res) => {
      res.redirect('https://daawesomep.github.io/face-game')
    })
